			...
			<div class="w3-container">
				<c:forEach var="alist" items="${ alist }" varStatus="sts">
					<ul class="w3-ul w3-card-4">
						<li id="detailemp${sts.index}" class="w3-bar"><span
							...
							<div id="ajaxApplyList" class="w3-bar-item"
								onclick="selectApplyList('${alist.EMPLOYMENT_ID}')">
								<span class="w3-large">공고명: ${alist.EMPLOYMENT_TITLE}</span><br>
								<span class="w3-large">공고내용: ${alist.EMPLOYMENT_CONTENTS}</span><br>채용공고기간${ alist.EMPLOYMENT_TIME }
								&nbsp;&nbsp; <span>지원시간: ${alist.APPLY_DATE} </span>
							</div></li>
					</ul>
				...
				$
						.ajax({
							...
							success : function(data) {
								$("#appendemp" + eid).empty();
								$("#appendemp" + eid).append(
										data.employment_title + "<br>");
								$("#appendemp" + eid).append(
										data.employment_contents + "<br>");
								$("#appendemp" + eid).append(
										data.hope_career + "<br>");
								$("#appendemp" + eid).append(
										data.hope_graduate + "<br>");
								$("#appendemp" + eid).append(
										data.hope_job_position + "<br>");
								$("#appendemp" + eid).append(
										data.hope_job + "<br>");
								$("#appendemp" + eid).append(
										data.hope_location + "<br>");
								$("#appendemp" + eid).append(
										data.hope_work_type + "<br>");
								$("#appendemp" + eid).append(
										data.employment_prefer + "<br>");
